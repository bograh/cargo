...
- ✅ **One gate, not two.** The pipeline deliberately performs no safety validation: checking the file as written cannot be sound, and a second weaker check invites being mistaken for the boundary — which is how the original bypasses got in. It keeps only the two Docker-free checks users actually hit (the compose file exists; it defines the named service). The single gate lives in `Apply`, ahead of the strategy split, and an `Apply`-level test asserts an unsafe file starts no containers at all
